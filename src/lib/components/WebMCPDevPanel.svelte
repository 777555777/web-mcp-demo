<script lang="ts">
	...
	function buildSystemPrompt(): string {
		...
		return [
			...
			'For the first reply to each new user request, always send a short natural-language acknowledgement first, for example "Okay, I will do that." or "Ok, im on it.".',
			'The first reply to a new user request must not contain any tool calls.',
			'Only after that acknowledgement may you send tool calls in a following assistant turn.',
			'This is important to ensure the user sees a response and builds trust that you understood their request and are taking action, even if the tool calls take a few seconds to complete.',
