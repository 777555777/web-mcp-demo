...
* It detects tools registered through both imperative methods, such as `registerTool()` and declarative methods using HTML form annotations.  
...
2. The extension indicates there’s a tool registered on the page:   
...
* **Principle: Tool registration management**  
  * Imperative tools can be registered and unregistered dynamically with `registerTool` and  `unregisterTool`.  
  * With the declarative API, a tool can be dynamically registered or unregistered by adding or removing the `toolName` and  `toolDescription` attributes on a form.  
  * Register tools when they make sense for a page state, and unregister when they don’t make sense anymore.
